...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-2xl">
        <!-- En-tête -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Modifier Commande #{{ $commande->id }}</h1>
            ...
            <form action="{{ route('commandes.update', $commande) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $commande->name) }}" required
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <select name="statut_commande" id="statut_commande" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            <option value="en_attente" {{ old('statut_commande', $commande->statut_commande) == 'en_attente' ? 'selected' : '' }}>En attente</option>
                            <option value="confirmee" {{ old('statut_commande', $commande->statut_commande) == 'confirmee' ? 'selected' : '' }}>Confirmée</option>
                            <option value="expediee" {{ old('statut_commande', $commande->statut_commande) == 'expediee' ? 'selected' : '' }}>Expédiée</option>
                            <option value="livree" {{ old('statut_commande', $commande->statut_commande) == 'livree' ? 'selected' : '' }}>Livrée</option>
                            <option value="annulee" {{ old('statut_commande', $commande->statut_commande) == 'annulee' ? 'selected' : '' }}>Annulée</option>
                        </select>
